...
- Use SMTP Authentication --> password  : generated password (without the spaces!)
- tick "Use SSL"
- SMTP Port : 465
...
- Configure Instance Details:
 ...
 ```
 For a strict/production setup: If you don't want to open all ports, you specifically need to open these ports in the Master node's Security Group:
...
And allow Weave/Flannel/Calico network ports (like UDP 8472 or TCP/UDP 179 depending on the CNI you used).
```
...
```
3. To Load Kernel Modules and Configure containerd: (master and worker nodes)
```
...
Note: In case you are in root, exit from it (use whoami and exit) and run below commands.
```
mkdir -p $HOME/.kube
...
```
example:
sudo kubeadm join 172.31.14.235:6443 --token vaorxq.q0kfn1imlf2wayni \
        --discovery-token-ca-cert-hash sha256:743611d499ccb5a775b6246e37e9aabeac6e5f4f8090edf1a7c056caa3a86966 
```
...
Copy the config file to Jenkins master or the local file manager and save it:(master node)
```
